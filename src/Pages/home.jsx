...
import { setLoginAct, setLogoutUserAct } from '../store/auth/authslice';
...
function Home() {
  ...
  const { loginResp } = useSelector(({ auth }) => ({
  ...
  useEffect(() => {
    ...
    if (token) {
      setIsLoggedIn(true);
      setUsername(loginResp?.UserInfo?.UserName || "User");
    }
    fetchData();
  }, []);
  ...
  const handleMyLearning = () => navigate('/mylearning');
  const handleLogout = () => {
       ...
       {isLoggedIn ? (
         <div className="relative">
          <button
           ...
           className="w-24 h-12 m-5 bg-button-bg text-text rounded-md hover:bg-button-hover-bg"
           ...
           <button
             ...
             className="w-24 h-12 gap-5 bg-button-bg text-text rounded-md hover:bg-button-hover-bg"
           >
             {username}
           </button>
...
{/* Categories */}
<div className="bg-card-background flex items-center w-full m-5 p-5 rounded-lg">
